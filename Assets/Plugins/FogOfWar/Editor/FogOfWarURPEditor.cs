    ...
    public static class FogOfWarURPEditor
    {
        ...
        public static void TryFixInstallIssues()
        {
            ...
            foreach (string guid in AssetDatabase.FindAssets("t:ScriptableRendererData", _searchInFolders))
            {
                ...
                if (renderdata is UniversalRendererData urenderdata)
                {
                    if (urenderdata.copyDepthMode != CopyDepthMode.AfterOpaques)
                    {
                        urenderdata.copyDepthMode = CopyDepthMode.AfterOpaques;
                        EditorUtility.SetDirty(renderdata);
                    }
#if UNITY_2021_2_OR_NEWER
                    if (urenderdata.intermediateTextureMode != IntermediateTextureMode.Always)
                    {
                        urenderdata.intermediateTextureMode = IntermediateTextureMode.Always;
                        EditorUtility.SetDirty(renderdata);
                    }
#endif
                }
                else if (renderdata is Renderer2DData renderdata2d)
