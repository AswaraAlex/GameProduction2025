...
namespace FoW
{
    ...
    public class FogOfWarUnit : MonoBehaviour
    {
        ...
        [EnableIf(nameof(lineOfSightMask), EnableIfComparison.NotEqual, 0), Tooltip("If true, the unit's position will be snapped to the center of the fog pixel.")]
        ...
        FogOfWarShape CreateShape(FogOfWarTeam fow)
        {
            ...
            {
                var shape = GetShapeFromCache<FogOfWarShapeBox>();
                shape.texture = texture;
                shape.pointSample = !texture || texture.filterMode == FilterMode.Point;
                shape.hasTexture = texture;
                shape.maxLineOfSightDistance = centerPoint.magnitude + boxSize.magnitude * 0.5f;
                FillShape(fow, shape, rotateToForward);

                if (fow.renderType == FogOfWarRenderType.Software && fow.multithreaded && texture)
        ...
        public byte GetVisibility(FogOfWarTeam team, FogOfWarMap map, Vector2Int position)
        {
            return CreateShape(team).GetVisibility(map, position);
        }
        ...
        void OnDrawGizmosSelected()
        {
            ...
            if (fow == null)
            {
                foreach (FogOfWarTeam t in FindObjectsOfType<FogOfWarTeam>(true))
                {
                    if (t.team == team)
                    {
                        fow = t;
