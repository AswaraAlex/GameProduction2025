Shader "Hidden/FogOfWarURP"
{
	...
	float GetFogAmount(float2 fogpos, float rawdepth)
	{
		...
		float2 fogcombined = SAMPLE_TEXTURE2D(_FogTex, sampler_FogTex, fogpos).rg;
		float visiblefog = fogcombined.r;
		float partialfog = fogcombined.g;
			...
			if (visiblefog < 0.5)
				...
				fog = _PartialFogAmount;
			else
				fog = 1;
		#else// STYLE_DEFAULT
			float fog = lerp(visiblefog, partialfog, _PartialFogAmount);
		#endif
	...
	float4 FogShader(FogData fogdata)
	{
		...
		float4 halfcolor = lerp(fogdata.backgroundColor, _MainFogColor, _MainFogColor.a);
		if (fogdata.fogAmount > _PartialFogAmount)
			return lerp(halfcolor, fogcolor, InverseLerp(_PartialFogAmount, 1, fogdata.fogAmount));
		return lerp(fogdata.backgroundColor, halfcolor, InverseLerp(0, _PartialFogAmount, fogdata.fogAmount));
	}
